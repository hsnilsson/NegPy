...
- Source loaders (`negpy/infrastructure/loaders/`) own `load_bounded_preview(...)`. It returns an oriented RGB image within the requested long edge, or `None` when the loader cannot keep the decode bounded. Automatic thumbnails never use a full camera RAW demosaic. A loader path opts into neighboring linear-preview prefetch with `supports_cancellable_linear_preview(...)` only when its decode is bounded and checks cancellation between units of work.
